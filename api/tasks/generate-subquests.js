// POST /api/tasks/generate-subquests — OpenAI subquest generation
import OpenAI from 'openai';
import { withAuth } from '../_lib/auth.js';
import { withCors } from '../_lib/cors.js';

export default withCors(
  withAuth(async (req, res, _user) => {
...
["Prepare and annotate the training dataset", "Configure YOLO hyperparameters for fine-tuning", "Run training and monitor loss curves", "Calculate Precision, Recall, and F1 scores on the validation set"]`;
      ...
      const completion = await client.chat.completions.create({
        ...
        messages: [
          ...
          { role: 'user',   content: `Break down this task into 3-5 actionable sub-tasks:\n\n"${title.trim()}"` },
        ],
        temperature:     0.4,
        max_tokens:      500,
      ...
      try {
        ...
        if (!match) return res.status(500).json({ error: 'Failed to parse AI response. Try again.' });
